...
class wiki80DemoDataset(Dataset):
    def __init__(self, input_ids, attention_mask, labels, so, rel2id, tokenizer, mode, args) -> None:
        ...
        self.mode = mode
        self.args = args
        self.labelid_list = [a[0] for a in tokenizer([f"[class{i}]" for i in range(1, len(rel2id)+1)], add_special_tokens=False)['input_ids']]
    ...
    def __getitem__(self, index):
        ...
        if self.demo_mode == 'get':
            ...
            return tuple([torch.tensor(input_id), torch.tensor(attn_mask), torch.tensor(label), \
                            torch.tensor(so_item), torch.tensor(block_flag_for_demo), torch.tensor(demo_mask_features)])
        else:
            return tuple([self.input_ids[index], self.attention_mask[index], \
                            self.labels[index], self.so[index]])
