...
def main():
    ...
    model_checkpoint = pl.callbacks.ModelCheckpoint(monitor=metric_name, mode="max" if not args.pretrain else "min",
        filename='{epoch}-{Eval/hits10:.2f}' if not args.pretrain else None,
        dirpath=os.path.join("output", args.data_dir.split("/")[-1]),
        save_weights_only=True,
        every_n_train_steps=50000 if args.pretrain else None
    )
