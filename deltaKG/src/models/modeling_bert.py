...
class BertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        if hasattr(self.config, 'kb_layer'):
            if isinstance(self.config.kb_layer, str):
                kb_layer = [int(i) for i in self.config.kb_layer.split(',') if i != '']
            else:
...
@add_start_docstrings("""Bert Model with a `language modeling` head on top.""", BERT_START_DOCSTRING)
class BertForMaskedLMForCaliNet(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))
            pos = (input_ids == self.config.mask_token_id).nonzero(as_tuple=True)
            masked_lm_loss = loss_fct(prediction_scores[pos[0], pos[1], self.config.decoder_start_token_id:self.config.decoder_end_token_id], labels.view(-1))
...
bert_mapping = {
  'KGEditor': EXBertForMaskedLM,
  'CaliNet': BertForMaskedLMForCaliNet,
}